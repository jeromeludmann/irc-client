...
export const RECEIVE_NOTICE_FROM_CHANNEL = 'MESSAGE/RECEIVE_NOTICE_FROM_CHANNEL'
...
export const messageReceivers: {
  ...
  NOTICE: (serverKey, prefix: Prefix, params): ReceiveNoticeActions => {
    ...
    return isChannel(params[0])
      ...
      : {
          ...
          route: { serverKey, bufferKey: BufferKey.ACTIVE },
        }
  },
  ...
  PING: (serverKey, _: Server, params): ReceivePingFromServerAction => ({
  ...
  PRIVMSG: (serverKey, user: User, params): ReceivePrivmsgAction => ({
    ...
    route: {
      serverKey,
      bufferKey: isChannel(params[0]) ? params[0] : user.nick,
    },
  }),

  '004': (serverKey, _: Server, params): ReceiveReplyMyInfoAction => {
